...
export default function EventsPage() {
  ...
  const fetchPublished = useCallback(async () => {
    ...
    try {
      ...
      const registrationEventIds = (Array.isArray(regRes.data) ? regRes.data : [])
        .map((r) => r?.event?._id)
        .filter(Boolean)
        .map((id) => String(id));
      setMyRegs([...new Set(registrationEventIds)]);
  ...
  const myRegEvents = events.filter((ev) => myRegs.includes(String(ev._id)));
  ...
  const handleUnregister = async (ev) => {
    ...
    try {
      ...
      setMyRegs((prev) => prev.filter((id) => id !== String(ev._id)));
    } catch (err) {
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
              {filtered.map((ev) => (
                <EventCard
                  key={ev._id}
                  event={ev}
                  isRegistered={myRegs.includes(String(ev._id))}
                  onRegister={handleRegister}
                  onUnregister={handleUnregister}
