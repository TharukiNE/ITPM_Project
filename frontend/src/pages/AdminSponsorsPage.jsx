...
export default function AdminSponsorsPage() {
  ...
  const handleNextPage = () => {
    ...
    if(form.email && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(form.email)) return setFormError("Invalid email address.");
    if(form.password.length > 0 && form.password.length < 6) return setFormError("Password must be at least 6 characters.");
    setFormError("");
    setModalPage(2);
  };
